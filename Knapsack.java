...
//		}
		//Start of threading
		int [][] decrypted = new int[message.length][];
		ExecutorService service = Executors.newFixedThreadPool(encrypted.length);
		List<Future> allFutures = new ArrayList<>();
		for(int i = 0; i<encrypted.length; i++) {
			Future<int []> future = service.submit(new DecryptThread(encrypted[i],privateS, inverse,modulus));
		...
		while(alive){
			alive = false;
			for(int k = 0; k < allFutures.size() && alive == false; k++) {
				if(!allFutures.get(k).isDone()) {
					alive = true;
				}
			}
		}
